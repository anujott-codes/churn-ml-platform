...
# test load_model failure with missing file
def test_load_model_failure(tmp_path):
    evaluator = ModelEvaluator(
...
def test_compute_precision_at_k(tmp_path):
    evaluator = ModelEvaluator(
        model_path=tmp_path / "model.pkl",
        test_data_path=tmp_path / "test.csv",
        reports_dir=tmp_path,
    ...
    precision_at_k = evaluator.compute_precision_at_k(y_true, y_proba)
